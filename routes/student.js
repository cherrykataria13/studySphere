...
router.post('/signup',(request,response)=>{
    ...
    database.query(queryy,[user.email],(error,results)=>{
        if(!error){
            ...
            if(results.length<=0){
                queryi = "insert into users(first_name,last_name,contact_number,email, password) values(?,?,?,?,?)"
                database.query(queryi,[user.firstName,user.lastName,user.contactNumber,user.email,user.password],(error,results)=>{
                    if(!error){
        ...
        if(!error){
            if(results.length<=0){
                ...
                var mailOptions ={
                    ...
                    html : '<p><b> Your login details for Study Sphere website are : </b><br><b>Email : </b>'+results[0].email+'<br><b>Password : </b>'+results[0].password+'<br></p>'
                };
...
router.get('/getDetails',authentication.authenticateToken,(request,response)=>{
    const email = response.locals.email;
    var queryi = "select first_name, last_name, email, contact_number from Users where email = ?";
